...
export default function DashboardLayout({ children }: { children: React.ReactNode }) {
  ...
  const { isLoading, user, role, menuItems, isAuthenticated } = useRole();
  ...
  return (
    ...
    >
      <DashboardLayoutContent setSidebarWidth={setSidebarWidth} menuItems={menuItems} user={user} role={role}>
        {children}
...
function DashboardLayoutContent({ children, setSidebarWidth, menuItems, user, role }: ContentProps) {
            ...
            <div className="flex items-center gap-3 px-2 w-full">
              ...
              {!isCollapsed && (
                <div className="flex items-center gap-2 min-w-0">
                  <button
                    onClick={() => setLocation("/")}
                    className="flex items-center gap-2 font-semibold tracking-tight truncate hover:opacity-80 transition-opacity"
                  >
                    <JobTrunkLogo />
                  </button>
                </div>
              ...
              {menuItems.map((item) => {
                ...
                return (
                  <SidebarMenuItem key={item.path}>
                    ...
                    >
                      <IconComp className={`h-4 w-4 ${isActive ? "text-primary" : ""}`} />
              ...
              <DropdownMenuTrigger asChild>
                <button className="flex items-center gap-3 rounded-lg px-1 py-1 hover:bg-accent/50 transition-colors w-full text-left group-data-[collapsible=icon]:justify-center">
                  ...
                  <div className="flex-1 min-w-0 group-data-[collapsible=icon]:hidden">
                    ...
                    <p className="text-xs text-muted-foreground truncate mt-1.5 capitalize">{role}</p>
                  </div>
                </button>
              </DropdownMenuTrigger>
